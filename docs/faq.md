...
Yes, through your Gemini subscription and the [Aside](https://aside.com) browser. The `codex` backend needs a paid ChatGPT plan, so on a host without one the keyless path is the [`gemini-web-image`](../.claude/skills/gemini-web-image/SKILL.md) skill: it drives the Gemini web app through `aside repl` using the Gemini account you signed in to inside Aside, and writes into the same `image_prompts.json` the rest of the pipeline reads. It runs on macOS, Linux, and Windows x64, and the Gemini UI language does not matter. Rows go four at a time in one `aside repl` call, which opens a tab per row, submits and downloads them one at a time while Gemini generates all four in parallel, and closes every tab before it returns. Measured on 2026-09-23: one image in 46 seconds, five images (a batch of four plus one) in 128 seconds, all at the original size — 2752×1536 for 16:9, 2400×1792 for 4:3, 2048×2048 for 1:1 — with five distinct checksums. The serial cost is the download of each original, 11–17s, so a batch of four takes about 100s and ten images about four minutes. The Aside window switches tabs while it works; leave it alone until the run finishes.

Three things to know before you rely on it. The run leaves one Chrome tab group per image (`agent:ppt-master-image-N`), because the isolation that keeps rows from overwriting each other is one bridge session per row and the bridge names a group after the session; a ten-image deck is ten groups until they are closed. It automates a logged-in web session, which is generally outside Google's terms — your account, your call. And it is a browser path: the page's layout is not a stable API, so it needs occasional maintenance in a way the CLI and API backends do not.
