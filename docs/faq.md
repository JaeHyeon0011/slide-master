...
Yes, through your Gemini subscription and the [Aside](https://aside.com) browser. The `codex` backend needs a paid ChatGPT plan, so on a host without one the keyless path is the [`gemini-web-image`](../.claude/skills/gemini-web-image/SKILL.md) skill: it drives the Gemini web app through `aside repl` using the Gemini account you signed in to inside Aside, and writes into the same `image_prompts.json` the rest of the pipeline reads. The Gemini UI language does not matter. **It is built to run in the background**: no step waits on the window being painted, so the Aside window may be covered or minimized while you work. That matters on Windows, where Chromium stops painting covered windows — the likely reason an earlier version stalled there. The background runs below were measured on macOS; the first Windows run is the confirmation. Rows go four at a time in one `aside repl` call, which opens a tab per row, submits them without bringing any forward, and downloads each original one at a time while Gemini generates all four in parallel. Measured on 2026-09-25 on macOS: ten images in 200s with the Aside app hidden and its window confirmed off-screen (the old Kimi bridge took 272s), four in 79s with it visible — every one at the original size (2752×1536 for 16:9, 2400×1792 for 4:3, 2048×2048 for 1:1), all distinct. Ten images take about three and a half minutes. Keep the Aside app open; a locked screen has not been tested.

Three things to know before you rely on it. Every tab the run opens is closed before it returns, and a row that fails is retried once in the same run, then left `Failed` with its reason in `last_error`, so rerunning the same command takes only what is left. It automates a logged-in web session, which is generally outside Google's terms — your account, your call. And it is a browser path: the page's layout is not a stable API, so it needs occasional maintenance in a way the CLI and API backends do not.
